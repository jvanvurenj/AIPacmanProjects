...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in livingGhostPositionDistributions:
            here = i.argMax()
            ##still need to figure out how to get the distance
            far = self.distancer.getDistance(pacmanPosition, here)
            if far < tempmax:
                gpos = here
                tempmax = far
        tempmax = float("inf")
        ...
        for i in legal:
            temppos = Actions.getSuccessor(pacmanPosition, i)
            ##also new distance needs to go here v
            far = self.distancer.getDistance(temppos, gpos)
            if far < tempmax:
                tempmax = far
                action = i
        return action
